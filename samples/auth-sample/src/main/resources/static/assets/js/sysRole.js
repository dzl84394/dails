	...
	$.ajax({
        ...
        success : function(result) {
		    ...
		    for (var i = 0; i <  result.data.records.length; i++) {
				...
				temp += "<tr>"
                 + "<td>"+index.id+"</td>"
                 + "<td>"+index.roleCode+"</td>"
                 + "<td>"+index.roleName+"</td>"
                 + "<td>"+index.roleType+"</td>"

				+"<td><a class='btn btn-info btn-sm' href='/sysRole/showView?id="+index.id+"'>展示</a>"
				+"<a class='btn btn-primary btn-sm' href='/sysRole/editView?id="+index.id+"'>编辑</a>"
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var index = result.data;
               ...
               $("#id").text( index.id);
  $("#roleCode").text( index.roleCode);
  $("#roleName").text( index.roleName);
  $("#roleType").text( index.roleType);
...
function editshow(){
    $.ajax({
        ...
        success: function (result) {
            ...
            var index = result.data;
              $("#id").val( index.id);
  $("#roleCode").val( index.roleCode);
  $("#roleName").val( index.roleName);
  $("#roleType").val( index.roleType);
