...
public class Branch2 extends Example {

	static String defFile = "/mongodb-restController";
...
//		addLauncher();
	...
	public void initJsp() {
		...
		createJsp(indexFile, name, defFile + "/views/index.html");
		createJsp(newFile, name, defFile + "/views/new.html");
		createJsp(editFile, name, defFile + "/views/edit.html");
		createJsp(showFile, name, defFile + "/views/show.html");
	}

	private void addBaseService() {
		// branch1 添加 Launcher.java

		File file = new File(getWorkUrl() + "/service/BaseService.java");
		if (!file.exists()) {
			try {
				file.createNewFile();
				System.out.println("新增文件: " + file.getName());
				BufferedWriter output = new BufferedWriter(new FileWriter(file));
				String string = readFile(defFile + "/BaseService.txt");
...
//				System.out.println("新增文件: " + file.getName());
//				BufferedWriter output = new BufferedWriter(new FileWriter(file));
//				String string = readFile(defFile+"/Launcher.java");
//
//				String myurl = getUrl().replaceAll("/", ".");
//				string = string.replaceAll("\\$\\{url\\}", myurl);
//				output.write(string);
...
//	}
		...
		if (!file.exists()) {
			try {
				...
				if (name.equals("index.html")) {
					for (int i = 0; i < filedNames.length; i++) {
						if (!filedNames[i].equals("serialVersionUID")) {
							...
							temp2 += "										<td th:text='\\$\\{s." + filedNames[i]
									+ "\\}'></td>\n";
						}
					}
				...
				if (name.equals("new.html")) {
					for (int i = 0; i < filedNames.length; i++) {
						if (!filedNames[i].equals("serialVersionUID") && !filedNames[i].equals("id")) {
							temp += "<div class='form-group col-lg-12'>" + "						<label for='"
									+ filedNames[i] + "' class='col-sm-2 control-label'>" + filedNames[i] + "</label>"
									+ "						<div class='col-sm-10'>"
									+ "							<input type='text' class='form-control' name='"
									+ filedNames[i] + "' id='" + filedNames[i] + "' th:value='${obj." + filedNames[i]
									+ "}' />" + "						</div>" + "					</div>";
						}
					}
				...
				if (name.equals("show.html")) {
					for (int i = 0; i < filedNames.length; i++) {
						if (!filedNames[i].equals("serialVersionUID")) {
							temp += "	<div>  \n" + "		<span class='label label-default'>" + filedNames[i]
									+ "</span> \n <span th:text='\\$\\{obj." + filedNames[i] + "\\}'" + "></span>	</div> \n";
						}
					}
	...
	public String readFile(String url) {
		...
		try {
			...
			/*
			 * file = ResourceUtils.getFile(url2);// FileReader reader = new
			 * FileReader(file);// 定义一个fileReader对象，用来初始化BufferedReader BufferedReader
			 * bReader = new BufferedReader(reader);// new一个BufferedReader对象，将文件内容读取到缓存
			 * String s = ""; while ((s = bReader.readLine()) != null) {//
			 * 逐行读取文件内容，不读取换行符和末尾的空格 sb.append(s + "\n");// 将读取的字符串添加换行符后累加存放在缓存中 }
			 * bReader.close();
			 */
